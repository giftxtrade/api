...
func (service *UserService) Find(key string) types.User {
	var user types.User
	service.DB.Table(TABLE).Where("id = ? OR email = ?", key, key).First(&user)
	return user
}

func (service *UserService) FindByEmail(email string) types.User {
	var user types.User
	service.DB.Table(TABLE).Where("email = ?", email).First(&user)
	return user
}

func (service *UserService) FindById(id string) types.User {
	var user types.User
	service.DB.Table(TABLE).Where("id = ?", id).First(&user)
	return user
}

func (service *UserService) FindByIdAndEmail(id string, email string) types.User {
	var user types.User
	service.DB.Table(TABLE).Where("id = ? AND email = ?", id, email).First(&user)
	return user
}

func (service *UserService) FindByIdOrEmail(id string, email string) types.User {
	var user types.User
	service.DB.Table(TABLE).Where("id = ? OR email = ?", id, email).First(user)
	return user
}
...
func (service *UserService) Create(create_user *types.CreateUser) types.User {
	...
	service.DB.Table(TABLE).Create(&user)
	return user
}